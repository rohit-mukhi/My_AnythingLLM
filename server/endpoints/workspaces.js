  ...
  app.post(
    ...
    async function (request, response) {
      try {
        ...
        // If markdown content exists, replace the original file with markdown content
        let mdFilename = null;
        if (markdownContent) {
          const hotdirPath =
          ...
          mdFilename = originalname.replace(/\.[^.]+$/, "") + ".md";
          const originalFilePath = path.join(hotdirPath, originalname);
          const mdFilePath = path.join(hotdirPath, mdFilename);
          ...
          // Now process the markdown file instead
          const mdProcessResult = await Collector.processDocument(mdFilename);
          if (!mdProcessResult.success) {
            console.warn(`Failed to process markdown file: ${mdProcessResult.reason}`);
          }
        }

        Collector.log(
          `Document ${originalname} uploaded, converted to markdown and successfully processed. It is now available in documents.`
        );
