    ...
    async function uploadFile() {
      ...
      if (!response.ok) {
        ...
        if (data?.markdownContent) {
          ...
          setMdFilename(data?.mdFilename);
          setShowPreview(true);
        }
        onUploadSuccess();
      }

      // Don't auto-dismiss if markdown content was converted - only dismiss when user closes preview
      if (!data?.markdownContent) {
        setTimeout(() => {
      ...
      >
        ...
        {status === "complete" && markdownContent && (
          <button
            ...
            className="flex items-center gap-x-1 text-xs text-white/60 hover:text-white transition-colors"
